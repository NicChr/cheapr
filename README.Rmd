...
-   Coercion between the R API and C API may not always match,
    especially for data frames. This is something that is likely to be
    improved in the future.

-   Because cheapr tries to treat data frames as vectors wherever
    possible, functions like `vec::length()` return the number of rows
    of a data frame. Use `vec::old_length()` to match `Rf_xlength()`
