...
clean, fast and safe code without sacrificing any one of these core design 
principles. 

cheapr includes both an R API in the usual form of an R package, as well as 
a C++ API for writing C++ code. It can be used interchangeably with the
R C API, Rcpp and cpp11 because all return types are either `SEXP` (R object in C) or
basic C/C++ types. The biggest requirement is that C++17 or later is needed.
...
To install cheapr and make use of its C++ functionality you'll need C++17 or later.

For help on getting started with C++ in R, see [Getting started with cpp11](https://cpp11.r-lib.org/articles/cpp11.html)

All the public user-facing C++ code is included in inst/include. To make use
of the API, simply include the cheapr API header file.

After this you have to link to cheapr either via the description file if writing an
R package 

`LinkingTo: cheapr`

or by including the cpp11 tag `[[cpp11::linking_to("cheapr")]]` in your C++ code.
