...
+---------------------+---------------------+---------------------+
| Feature             | R C API             | C++/cpp11           |
+=====================+=====================+=====================+
| Protecting objects  | Pros: Fast          | Pros: No manual     |
| from GC             |                     | protection needed.\ |
|                     | Cons: Manually      | Cons: Slow if       |
|                     | protect with        | re-protecting an    |
|                     | PROTECT/UNPROTECT   | object many times   |
|                     |                     | (e.g. \>10k)        |
+---------------------+---------------------+---------------------+
| Safety              | No need to use      | Must use `r_safe`   |
|                     | `r_safe` when not   | on either cheapr    |
|                     | using C++ objects.  | functions or R C    |
|                     |                     | API functions when  |
|                     |                     | C++ objects are     |
|                     |                     | created.            |
+---------------------+---------------------+---------------------+
| Type declarations   | All R objects are   | Pros: Uses proper   |
|                     | declared as SEXP\   | C++ types, e.g.     |
|                     | Pros: Sometimes     | integers, doubles,  |
|                     | this ambiguity is   | etc, making the     |
|                     | useful.\            | return type clear.\ |
|                     | Cons: Unclear what  | Cons: Not flexible  |
|                     | the return type     | if needing an input |
|                     | should be.          | which can be either |
|                     |                     | integer or double   |
|                     |                     | in which case       |
|                     |                     | cpp11::sexp can be  |
|                     |                     | used.               |
+---------------------+---------------------+---------------------+
| Exporting C/C++     | Use                 | Use                 |
| functions to R      | [[cpp11::register]] | [[cpp11::register]] |
+---------------------+---------------------+---------------------+
| Using               | No need to use      | You must use        |
| r_safe/cpp11::safe  | `r_safe` when       | `r_safe` **on**     |
|                     | **ONLY** using      | cheapr/R C API      |
|                     | cheapr + R C API    | functions **IF**    |
|                     |                     | you have used any   |
|                     |                     | cpp11/C++ code.     |
+---------------------+---------------------+---------------------+
