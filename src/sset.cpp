...
SEXP cpp_sset_range(SEXP x, R_xlen_t from, R_xlen_t to, R_xlen_t by){
  ...
  case REALSXP: {
    ...
    if (double_loop){
      ...
      if (by > 0){
        memmove(&p_out[0], &p_x[istart - 1], static_cast<uint_least64_t>(in_bounds_size * sizeof(double)));
        OMP_FOR_SIMD
...
SEXP sset_vec(SEXP x, SEXP indices, bool check){
  ...
  if (check){
    ...
    if (Rf_xlength(x) > integer_max_){
      ...
      const double* RESTRICT pind = REAL(indices);
      ...
      const int* RESTRICT pind = INTEGER(indices);
      switch ( xtype ){
