...
SEXP cpp_rep(SEXP x, SEXP times){
  ...
  if (n_times == 1){
    out_size = n * vector_ptr<r_int_t>(times)[0];
    SEXP out = SHIELD(cpp_rep_len(x, out_size));
    YIELD(2);
    return out;
  } else {
    r_int_t *p_times = vector_ptr<r_int_t>(times);
    if (n_times != n){
      YIELD(1);
...
SEXP cpp_rep_each(SEXP x, SEXP each){
  ...
  if (Rf_length(each) == 1){
    if (vector_ptr<r_int_t>(each)[0] == 1){
      YIELD(NP);
      return x;
    }
...
SEXP cpp_df_col_c(SEXP x, bool recycle, bool name_repair){
  ...
  SEXP df_nrows = SHIELD(as_vector(as<r_int_t>(common_size))); ++NP;
