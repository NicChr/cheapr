test_that("if else", {
  ...
  expect_error(if_else_(x == 3, 1:2, 1))
  expect_error(if_else_(x == 3, 1, 1:2))
  expect_error(if_else_(x == 3, 1, 0, na = 1:2))
  expect_error(if_else_(1L, 1, 0))
  ...
  expect_identical(
    if_else_(x == 3, .Date(0), .Date(1)),
    .Date(ifelse(x == 3, 0, 1))
  )

  expect_identical(
    if_else_(x == 3, list(1:3), list(3:1)),
    ifelse(is.na(x), list(NULL), ifelse(x == 3, list(1:3), list(3:1)))
  )

  expect_identical(
    if_else_(x == 3, list(1:3), list(3:1), na = list(NA_real_)),
    ifelse(is.na(x), list(NA_real_), ifelse(x == 3, list(1:3), list(3:1)))
  )

  expect_identical(
    if_else_(z == z[1], z[2], z[1]),
  ...
  expect_identical(
    if_else_(is.na(x) | !is.na(x), Inf, -Inf),
  ...
  expect_identical(
    if_else_(rep(NA, length(x)), Inf, -Inf),
  ...
  expect_identical(
    if_else_(is.na(x) | !is.na(x), pos_inf, neg_inf),
  ...
  expect_identical(
    if_else_(rep(NA, length(x)), pos_inf, neg_inf),
  ...
  expect_identical(
    if_else_(is.na(x), iris$Species[1], iris$Species[60]),
    structure(
