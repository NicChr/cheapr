...
//   // Copy SEXP pointers to vector for sorting
//   std::vector<SEXP> ptrs(n);
//   for (R_xlen_t i = 0; i < n; ++i) {
//     ptrs[i] = x.get(i);
//   }
  
//   // Sort pointers (std::sort uses introsort, O(n log n))
//   std::sort(ptrs.begin(), ptrs.end());
  
//   // Count unique by comparing consecutive pointers
//   R_xlen_t n_unique = 0;
//   SEXP prev = R_NilValue;
//   for (R_xlen_t i = 0; i < n; ++i) {
//     if (ptrs[i] != prev) {
//       ptrs[n_unique++] = ptrs[i];
//       prev = ptrs[i];
//     }
//   }
